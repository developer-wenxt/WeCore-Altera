const pgitFacOutService = require('../services/pgitFacOutService');
...
exports.getAll = async (req, res, next) => {
  try {
    ...
    const result = await pgitFacOutService.getAll(filters, queryOptions);
...
exports.create = async (req, res, next) => {
  try {
    const result = await pgitFacOutService.create(req.body);
...
exports.update = async (req, res, next) => {
  try {
    const result = await pgitFacOutService.update(req.params.id, req.body);
...
exports.deleteItem = async (req, res, next) => {
  try {
    const result = await pgitFacOutService.deleteItem(req.params.id);
  ...
  try {
    ...
    if (!foPolSysId) {
      return res.status(400).json({
        success: false,
        message: "foPolSysId is required"
      });
    }
