const pgitFacCommTaxService = require('../services/pgitFacCommTaxService');
...
exports.getAll = async (req, res, next) => {
  try {
    ...
    const result = await pgitFacCommTaxService.getAll(filters, queryOptions);
...
exports.create = async (req, res, next) => {
  try {
    const result = await pgitFacCommTaxService.create(req.body);
...
exports.update = async (req, res, next) => {
  try {
    const result = await pgitFacCommTaxService.update(req.params.id, req.body);
...
exports.deleteItem = async (req, res, next) => {
  try {
    const result = await pgitFacCommTaxService.deleteItem(req.params.id);
...
exports.getByPolSysId = async (req, res, next) => {
  try {
    const { fctFpsSysId } = req.query;

    if (!fctFpsSysId) {
      return res.status(400).json({
        success: false,
        message: "fcFpsSysId is required"
      });
    }

    const result = await pgitFacCommTaxService.getByPolSysId(Number(fctFpsSysId));
