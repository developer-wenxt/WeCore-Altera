const pgitFacPlacePcDtlService = require('../services/pgitFacPlacePcDtlService');
...
exports.getAll = async (req, res, next) => {
  try {
    ...
    const result = await pgitFacPlacePcDtlService.getAll(filters, queryOptions);
...
exports.create = async (req, res, next) => {
  try {
    const result = await pgitFacPlacePcDtlService.create(req.body);
...
exports.update = async (req, res, next) => {
  try {
    const result = await pgitFacPlacePcDtlService.update(req.params.id, req.body);
...
exports.deleteItem = async (req, res, next) => {
  try {
    const result = await pgitFacPlacePcDtlService.deleteItem(req.params.id);
...
exports.getByPolSysId = async (req, res, next) => {
  try {
    const { fppdFpsSysId } = req.query;

    if (!fppdFpsSysId) {
      return res.status(400).json({
        success: false,
        message: "fppdFpsSysId is required"
      });
    }

    const result = await pgitFacPlacePcDtlService.getByPolSysId(Number(fppdFpsSysId));
